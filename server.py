...
def get_inventory_items(
    ...
    """Danh sách vật tư tồn kho thực tế từ ERP SQL Server (bảng erp_inventory_stocks)"""
    conn = get_connection()
    c = conn.cursor()
    try:
        where = []
        ...
        if q and q.strip():
            kw = f"%{q.strip()}%"
            where.append("(ItemCode LIKE ? OR ItemName LIKE ?)")
            params.extend([kw, kw])
        if wh and wh.strip():
            where.append("WarehouseID = ?")
            params.append(wh.strip())
        if status == "available":
            where.append("ClosingStock > 0")
        elif status == "out_of_stock":
            where.append("ClosingStock <= 0")
        elif status == "low_stock":
            where.append("ClosingStock > 0 AND ClosingStock <= 5")

        where_clause = ("WHERE " + " AND ".join(where)) if where else ""
        c.execute(f"SELECT COUNT(*) FROM erp_inventory_stocks {where_clause}", params)
        total = c.fetchone()[0]

        offset = (page - 1) * limit
        sql = f"""
            SELECT id, WarehouseID, ItemCode, ItemName, Unit, ClosingStock, SyncedAt
            FROM erp_inventory_stocks
            {where_clause}
...
def get_inventory_summary():
    """Thống kê tổng quan tồn kho theo từng kho"""
    conn = get_connection()
    c = conn.cursor()
    try:
        c.execute("""
            SELECT 
                ...
                SUM(CASE WHEN ClosingStock > 0 THEN 1 ELSE 0 END) as positive_rows,
                SUM(CASE WHEN ClosingStock <= 0 THEN 1 ELSE 0 END) as zero_rows,
                SUM(CASE WHEN ClosingStock > 0 AND ClosingStock <= 5 THEN 1 ELSE 0 END) as low_stock_rows,
                MAX(SyncedAt) as last_synced_at
            FROM erp_inventory_stocks
